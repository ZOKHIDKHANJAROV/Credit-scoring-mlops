...
app = FastAPI(title="AI Engineering Command Center Agent", version="0.5.0")
...
def build_agent() -> ToolCallingAgent:
    return ToolCallingAgent(provider=OpenAICompatibleProvider(), registry=build_default_registry(), max_rounds=4, audit_service=audit_service)

@app.get("/health")
def health() -> dict[str, str]: return {"status": "ok", "service": "ai-engineering-agent"}

@app.get("/api/v1/agent/tools")
def list_tools() -> dict[str, list[str]]: return {"tools": build_default_registry().names()}

@app.post("/api/v1/agent/run", response_model=AgentRunResponse)
def run_agent(request: AgentRunRequest) -> AgentRunResponse:
    try: result = build_agent().run(task=request.task)
    except Exception as exc: raise HTTPException(status_code=500, detail=str(exc)) from exc
    return AgentRunResponse(status=str(result.get("status", "unknown")), answer=str(result.get("answer", "")), tools_used=list(result.get("tools_used", [])), trace_id=str(result["trace_id"]))
...
def create_approval(request: ApprovalCreateRequest) -> ApprovalRequest:
    approval = ApprovalRequest(action=request.action, reason=request.reason, execution_plan=request.execution_plan)
    approval_store.create(approval)
    audit_service.record(AuditEventType.APPROVAL_REQUESTED, trace_id=approval.approval_id, action=approval.action, status=approval.status.value, payload={"reason": approval.reason, "execution_plan": approval.execution_plan})
    return approval

@app.get("/api/v1/approvals", response_model=list[ApprovalRequest])
def list_pending_approvals() -> list[ApprovalRequest]: return approval_store.list_pending()
...
def decide_approval(approval_id: str, decision: ApprovalDecision) -> ApprovalRequest:
    if decision.approval_id != approval_id: raise HTTPException(status_code=400, detail="Approval ID in path and body must match")
    try: approval = approval_store.decide(decision)
    except KeyError as exc: raise HTTPException(status_code=404, detail=str(exc)) from exc
    except ValueError as exc: raise HTTPException(status_code=409, detail=str(exc)) from exc
    audit_service.record(AuditEventType.APPROVAL_DECIDED, trace_id=approval_id, actor=decision.decided_by, action=approval.action, status=approval.status.value, payload={"approved": decision.approved, "comment": decision.comment})
    return approval
...
def execute_approval(approval_id: str) -> ApprovalRequest:
    approval = approval_store.get(approval_id)
    if approval is None: raise HTTPException(status_code=404, detail="Approval not found")
    if approval.action != "create_training_job": raise HTTPException(status_code=400, detail=f"Unsupported approval action: {approval.action}")
    try: approval = approval_store.mark_executing(approval_id)
    except ValueError as exc: raise HTTPException(status_code=409, detail=str(exc)) from exc
...
def list_audit_events(trace_id: str | None = Query(default=None), event_type: AuditEventType | None = Query(default=None), limit: int = Query(default=100, ge=1, le=1000)) -> list[AuditEvent]:
...
def get_trace(trace_id: str) -> list[AuditEvent]: return audit_store.list(trace_id=trace_id)

@app.get("/api/v1/audit/stats")
def audit_stats() -> dict[str, int]: return {"event_count": audit_store.count()}
